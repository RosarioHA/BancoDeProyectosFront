...
const ModalAbandonoFormulario = forwardRef(function ModalAbandonoFormulario({ isOpen, onClose, direction, goBack }, ref) {
  ...
  const renderLinkOrButton = () => {
    if (direction) {
      ...
      return (
        <button className="btn-secundario-s  text-decoration-underline me-3" onClick={handleBackClick}>
          Salir del formulario
        </button>
      );
    } else {
      return (
        <button className="btn-secundario-s text-decoration-underline me-3" onClick={handleBtnClick}>
          Salir del formulario
        </button>
      );
    }
  ...
  return (
    <div className={`modal ${isOpen ? 'open' : ''}`}>
      <div className="modal-content p-3">
        ...
        <div className="d-flex justify-content-between">
          {renderLinkOrButton()}
          <button className="btn-secundario-s text-decoration-underline me-3" onClick={handleCloseRef.current}>Seguir en el formulario</button>
        </div>
      </div>
    </div>
  );
